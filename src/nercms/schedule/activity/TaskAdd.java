...
public class TaskAdd extends SherlockActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 准备用户信息
		userID = MySharedPreference.get(TaskAdd.this,
		...
		btnPodPicker.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				startActivityForResult(intent,
						LocalConstant.TASK_POD_SELECT_REQUEST_CODE);
			}
		});
	...
	private void backDetect() {
		...
		if (!title.isEmpty() && String.valueOf(podID).isEmpty()
				&& taskContent.isEmpty() && mediaIndex == 0) {
			new AlertDialog.Builder(TaskAdd.this)
					.setTitle("确定退出")
	...
	private void createTask() {
		...
		// 判断是否为空
		if (title.isEmpty() || podID == 0 
				|| taskContent.isEmpty()) {
			new AlertDialog.Builder(TaskAdd.this).setTitle("无法发起新任务")
					.setMessage("请重新检查任务是否填写完整").setPositiveButton("确定", null)
					.create().show();
			return;
		}

		task = new AffairModel(taskID, 1, sponsorID, title, taskContent,
				beginTime, endTime, null, 1, beginTime, Constant.READ, 1);
		// 添加责任人
		task.setPerson(pod);
		// 判断附件
		if (mediaIndex != 0) { // 有附件
			...
			for (int i = 0; i < mediaIndex; i++) {
				...
				attach = new AffairAttachModel(taskID, media.getMediaType(),
						media.getMediaName());
				...
				// 开启上传
				new HttpUploadTask(tvUploadStatus, this).execute(mediaPath,
						uploadUrl);
			...
			// 联网发送任务到服务器
			webRequestManager.sendAffair(task);
		}
	}
	...
	private void initHandler() {
			...
			public void handleMessage(Message msg) {
				...
				case Constant.CREATE_AFFAIR_REQUEST_SUCCESS:
					...
					ArrayList<AffairModel> affairs = new ArrayList<AffairModel>(
							1);
					affairs.add(task);
					ThreadManager.getInstance().startSaveAffairThread(affairs,
							true, TaskAdd.this);
		...
		MessageHandlerManager.getInstance().register(handler,
				Constant.CREATE_AFFAIR_REQUEST_SUCCESS, "TaskAdd");
		MessageHandlerManager.getInstance().register(handler,
				Constant.CREATE_AFFAIR_REQUEST_FAIL, "TaskAdd");
		MessageHandlerManager.getInstance().register(handler,
				Constant.FILE_UPLOAD_FAIL, "TaskAdd");
		MessageHandlerManager.getInstance().register(handler,
				Constant.FILE_UPLOAD_SUCCESS, "TaskAdd");
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case LocalConstant.CAPTURE_IMAGE_REQUEST_CODE:
			if (resultCode == RESULT_OK) {
				...
				String imageName = originalUri.substring(originalUri
						.lastIndexOf("/") + 1);
				mediaList.add(new Media(Utils.MEDIA_TYPE_IMAGE, imageName,
						originalUri));
				// 显示所拍照片
				Uri uri = Uri.fromFile(file);
				int mediaID = mediaIndex++;
				loadMedia(imageContainer, mediaID, getThumbnailFromUri(uri),
						uri, TYPE_IMAGE);
		...
		case LocalConstant.CAPTURE_VIDEO_REQUEST_CODE:
			...
			// 生成视频缩略图
			Bitmap videoThumbnailBitmap = ThumbnailUtils.createVideoThumbnail(
					originalUri, Thumbnails.FULL_SCREEN_KIND);
		...
		case LocalConstant.SELECT_IMAGE_REQUEST_CODE:
			...
			if (data == null) {
				...
				if (!(originalUri.endsWith("jpg")
						|| originalUri.endsWith("gif")
						|| originalUri.endsWith("bmp") || originalUri
							.endsWith("png"))) {
					originalUri = "";
				...
				loadMedia(imageContainer, mediaID1,
						getThumbnailFromUri(thumbUri), thumbUri, TYPE_IMAGE);
				// 存储mediaId与imageOriginPath的映射
		...
		case LocalConstant.TASK_POD_SELECT_REQUEST_CODE:
			if (resultCode == RESULT_OK) {
				...
				podID = Integer.parseInt(data.getExtras().getString(
						"selected_id"));
			}
			break;
				...
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface arg0, int which) {
						...
						case 1:
							...
							imagePath = Utils.produceAttachDir(
									Utils.MEDIA_TYPE_IMAGE, taskID,
									TaskAdd.this);
						...
						case 2:
							...
							videoPath = Utils.produceAttachDir(
									Utils.MEDIA_TYPE_VIDEO, taskID,
									TaskAdd.this);
							...
							videoIntent.putExtra(
									MediaStore.EXTRA_DURATION_LIMIT, 30000);
	 ...
	 * @param MediaType
	 */
	public void loadMedia(nercms.schedule.layout.FixedGridLayout viewContainer,
			int mediaId, Bitmap thumbnail, final Uri uri, final int MediaType) {
	...
	public RelativeLayout WrapImgView(int mediaType, ImageView imgview,
		...
		// height);
		rl.setLayoutParams(new nercms.schedule.layout.FixedGridLayout.LayoutParams(
