...
public class MyLog {
	...
	private static String MYLOG_PATH_SDCARD_DIR = "/sdcard/nercms-Schedule/Log/";// 日志文件在sdcard中的路径
	...
	private static void writeLogtoFile(String mylogtype, String tag, String text) {// 新建或打开日志文件
		...
		File dir = new File(MYLOG_PATH_SDCARD_DIR, needWriteFiel
				+ MYLOGFILEName);
		if (!dir.exists()) {
			try {
				dir.createNewFile();
		...
		try {
			FileWriter filerWriter = new FileWriter(dir, true);// 后面这个参数代表是不是要接上文件中原来的数据，不进行覆盖
